...
public:
    ...
    struct ShaderData
    {
        ShaderData(Type type, const char* path) : type {type},path {path} {}
        Type type;
        const char* path;
    };
...
public:
    ...
    Shader(const std::vector<ShaderData> &shaders)
    {
        ...
        for (ShaderData shData : shaders)
        {
            try
            {
                // open files
                shFile.open(shData.path);
                ...
                shFile.close();
                // convert stream into string
                shContent = ShaderStream.str();
            }
            catch (std::ifstream::failure& e)
            {
                std::cout << "ERROR::SHADER::FILE_NOT_SUCCESFULLY_READ: " << e.what() << std::endl;
            }
            const char* shContentCStr = shContent.c_str();
            ...
            glShaderSource(shaderID, 1, &shContentCStr, NULL);
